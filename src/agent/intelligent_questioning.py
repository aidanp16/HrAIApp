...
class IntelligentQuestioning:
    ...
    def _init_context_patterns(self) -> Dict[str, Any]:
        """Initialize pattern recognition for context analysis"""
        return {
            "stage_indicators": {
            ...
            "role_indicators": {
                "engineering": ["developer", "engineer", "technical", "coding", "software", "backend", "frontend", "full stack"],
                "marketing": ["marketing", "growth", "acquisition", "brand", "content", "campaigns"],
                "sales": ["sales", "business development", "account", "revenue", "closing", "pipeline"],
                "executive": ["cto", "vp", "director", "head of", "chief", "leadership", "executive"]
            },
            "urgency_indicators": {
    ...
    def analyze_context(self, user_input: str, conversation_history: List[str] = None) -> Dict[str, Any]:
        ...
        # Analyze company stage indicators
        stage_scores = {}
        ...
        # Analyze role type indicators
        role_scores = {}
        for role, indicators in self.context_patterns["role_indicators"].items():
            score = sum(1 for indicator in indicators if indicator in user_input_lower)
            if score > 0:
        ...
        if role_scores:
            ...
            context["role_type"] = RoleType(best_role)
            context["confidence_scores"]["role_type"] = role_scores[best_role] / 3
        ...
        # Calculate specificity score (how detailed is the request)
        specific_terms = len([word for word in user_input.split() if len(word) > 4])  # Longer words often indicate specificity
        context["specificity_score"] = min(specific_terms / 10.0, 1.0)  # Normalize to 0-1
